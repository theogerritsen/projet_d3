const width = 700, height = 550;
	...
	var promises = [];
	promises.push(d3.json("data/canton-final-topo.json"));
	...
	Promise.all(promises).then(function(values) {
		...
		csv.forEach(function(e,i) {
			d3.select("#d" + e.KANTONSNUM)
				.attr("class", d => "department q" + quantile(+e.score) + "-9")
				.on("mouseover", function(d) {
					div.transition()        
						.duration(200)      
						.style("opacity", .9);
					div.html("<b>Région : </b>" + e.canton + "<br>"
							+ "<b>Département : </b>" + e.KANTONSNUM + "<br>"
							+ "<b>Population : </b>" + e.score + "<br>")
